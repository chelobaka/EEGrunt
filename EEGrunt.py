...
class EEGrunt:
    def __init__(self, path, filename, source, title="", params={}):
        ...
        if self.source == 'muse' or self.source == 'muse-lsl':
            ...
            if self.source == 'openbci-ganglion' or self.source == 'openbci-ganglion-openvibe':
                ...
                self.first_col_is_time = params[
                    "first_col_is_time"]  # Boolean - specifies whether the 1st column we read in is channel 1 or a channel
    ...
    def load_data(self):
        ...
        print("Session length (seconds): %f" % (len(self.t_sec) / self.fs_Hz))
        print("Last t_sec: %f" % self.t_sec[-1])
    ...
    def smooth(self, x, window_len=11, window='hanning'):
        ...
        if not window in ['flat', 'hanning', 'hamming', 'bartlett', 'blackman']:
            raise ValueError(
                "Invalid window type in smooth(). Must be one of 'flat', 'hanning', 'hamming', 'bartlett', or 'blackman'")
    ...
    def plot_coherence_fft(self, s1, s2, chan_a, chan_b):
        ...
        plt.xlabel("Frequency (Hz)")
        plt.title(self.plot_title(
            "Coherence between channels " + chan_a + " and " + chan_b + " in the " + str(config['band'][0]) + "-" + str(
                config['band'][1]) + " Hz band"))
        plt.grid(True)
